...
OUTPUT_FILE = "GEX公式全商品_Keepa調査_v4_4.xlsx"
...
def ask_choice(prompt, choices, default=None):
    """選択肢以外や必須欄の空入力なら、終了せず正しい入力まで聞き直す。"""
    normalized_choices = {str(choice).upper() for choice in choices}
    aliases = {
        "Y": "YES",
        "N": "NO",
        "はい": "YES",
    ...
    while True:
        ...
        normalized = aliases.get(value, aliases.get(upper_value, upper_value))
        if normalized in normalized_choices:
            return normalized

        choices_text = " / ".join(sorted(normalized_choices))
        if default is None:
            print(f"入力が必要です。{choices_text} のいずれかを入力し直してください。")
        else:
            print(
                f"入力内容を確認できません。{choices_text} のいずれかを入力するか、"
                f"空Enterで既定値 {str(default).upper()} を使用してください。"
            )


def ask_nonnegative_integer(prompt, default=None, allow_all=False):
    """0以上の整数（必要ならALL）になるまで聞き直す。"""
    while True:
        ...
        if default is not None:
            guide += f"（空Enterは {default}）"
        print(f"入力内容を確認できません。{guide}を入力し直してください。")


def ask_secret_nonempty(prompt):
    """秘密情報が空欄なら、その場で再入力を求める。"""
    while True:
...
reuse_master = ask_choice(
    "既存ファイルを再利用しますか？（推奨 YES／初回のみ NO）：",
    {"YES", "NO"},
)
...
if MASTER_REUSED:
    if IN_COLAB:
        ...
        while True:
            ...
            if excel_files:
                break
            print("Excelファイルが選択されていません。もう一度アップロードしてください。")
        master_source_file = excel_files[0]
    else:
        while not master_source_file:
            master_source_file = input("既存Excelのパス：").strip()
            if not master_source_file:
                print("パスが入力されていません。もう一度入力してください。")
...
scope_text = ask_choice(
    "対象区分（未入力はAQUA）：",
    scope_map.keys(),
    default="AQUA",
)
selected_sections = scope_map[scope_text]
...
jan_start = ask_nonnegative_integer(
    "分割実行の開始位置（通常は0、未入力は0）：",
    default=0,
)
jan_limit = ask_nonnegative_integer(
    "今回照合する最大JAN数（推奨200、全件はALL、未入力は200）：",
    default=200,
    allow_all=True,
)
...
answer = ask_choice(
    ...
    f"最低約{len(target_jans)}トークンを消費します。続ける場合は YES、中止は NO：",
    {"YES", "NO"},
)
...
API_KEY = ask_secret_nonempty("Keepa APIキーを入力してください：")

BASE_URL = "https://api.keepa.com"
keepa_session = requests.Session()
keepa_session.headers.update({"Accept-Encoding": "gzip", "User-Agent": "GEX-Research-v4.4"})
...
if df_matched.empty:
    ...
    bb_limit = ask_nonnegative_integer(
        "深掘り件数を入力（推奨200、全件はALL、スキップは0、未入力は200）：",
        default=200,
        allow_all=True,
    )
    ...
    if bb_limit is None:
        ...
        bb_asins = all_bb_asins[:bb_limit]

    print(f"深掘り対象：{len(bb_asins)} ASIN / 推定最大約{len(bb_asins) * 3}トークン")
    confirm_bb = "NO"
    if bb_asins:
        confirm_bb = ask_choice(
            "この件数で実行する場合はYES、実行しない場合はNO：",
            {"YES", "NO"},
        )
...
print("\n" + "=" * 72)
print("GEX公式カタログ起点 Keepa調査 v4.4 完了")
print("=" * 72)
